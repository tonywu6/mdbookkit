...
use mdbookkit::{emit_debug, env::is_logging, error::put_severity, logging::stderr};
    ...
    pub fn emit(self) {
        if is_logging() {
            for issue in self.issues {
                issue_to_traces(issue, &self.source, &self.tracer);
            }
        } else {
                ...
                .filter(|issue| tracing_level_enabled(issue.level.into()))
            {
                ...
                let report = issue_to_report(issue, &self.source);
...
pub fn issue_to_report<'a>(issue: IssueReport<'a>, source: impl SourceCode<'a>) -> Vec<Group<'a>> {
    macro_rules! snippet {
...
pub fn issue_to_traces<'a, S, F>(issue: IssueReport<'a>, source: S, emit: F)
where
    S: SourceCode<'a>,
    F: Fn(IssueLevel, Arguments<'_>),
{
    struct IssueFormatter<'a, S> {
        issue: IssueReport<'a>,
        source: S,
    }

    impl<'a, S> Display for IssueFormatter<'a, S>
    where
        S: SourceCode<'a>,
    {
...
macro_rules! emit_issue {
    () => {
        |level: $crate::diagnostic::IssueLevel, message: ::std::fmt::Arguments<'_>| match level {
            $crate::diagnostic::IssueLevel::Error => ::tracing::error!("{message}"),
            $crate::diagnostic::IssueLevel::Warning => ::tracing::warn!("{message}"),
            $crate::diagnostic::IssueLevel::Info => ::tracing::info!("{message}"),
            $crate::diagnostic::IssueLevel::Note => ::tracing::debug!("{message}"),
            $crate::diagnostic::IssueLevel::Help => ::tracing::info!("{message}"),
        }
    };
}
