...
- Diagnostic messages, such as for broken links, now [include source locations (file:line:column) when the preprocessor is in logging mode](/docs/src/rustdoc-links/configuration/environment-variables.md#mdbookkit_term_graphical).
...
- `MDBOOK_LOG` is now the environment variable to control logging. This variable also controls logging in the main `mdbook` program. Previously, the variable was ~~`RUST_LOG`~~.
  - Logging is now implemented through `tracing`. The `MDBOOK_LOG` variable therefore supports [all syntax supported by `tracing`][tracing_subscriber::filter::EnvFilter]. See [Logging](/docs/src/rustdoc-links/configuration/environment-variables.md#mdbook_log) for more information.
